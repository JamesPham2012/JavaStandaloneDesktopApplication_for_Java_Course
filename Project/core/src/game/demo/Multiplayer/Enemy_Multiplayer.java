...
public class Enemy_Multiplayer extends GameObj {
    ...
    public static Vector<Enemy_Multiplayer> enemy_arr=new Vector<>();

    public Enemy_Multiplayer(float x_c, float y_c, int Id, int Wave) {
        x = x_c;
        x_b = x_c;
        y = y_c;
        y_b = y_c;
    ...
    public static void render(SpriteBatch batch) {
        for (int i = 0; i < enemy_arr.size(); i++) {
            if (enemy_arr.elementAt(i).State) {
                ...
                enemy_arr.elementAt(i).renders(batch);
            }
        }
    }
    ...
    public static void Enemy_Reallo(float x_c, float y_c, int id, int Wave) {
        ...
        loop:     while (EnemyGen == 0) {
            ...
            enemy_arr.addElement(new Enemy_Multiplayer(x_c, y_c, id, Wave));// create a new enemy
    ...
    private void Execute(){
        State=false;
        id=0;
        Item_Multiplayer.drop(x,y,id);
        MultiplayerGame.point++;
    ...
    public void checksCollision(){
        for (int i=0;i<Bullet_Multiplayer.bullet_arr.size();i++) {
            if ((!Bullet_Multiplayer.bullet_arr.elementAt(i).isDed())&&
                    (Bullet_Multiplayer.bullet_arr.elementAt(i).id*id<0)&&
                    (Math.sqrt(Math.pow((double)Bullet_Multiplayer.bullet_arr.elementAt(i).getX()-x,2.0)+
                            Math.pow((double)Bullet_Multiplayer.bullet_arr.elementAt(i).getY()-y,2.0))<
                            (double)Bullet_Multiplayer.bullet_arr.elementAt(i).hitboxRadius+hitboxRadius)){
                value-=Bullet_Multiplayer.bullet_arr.elementAt(i).getValue();
                Bullet_Multiplayer.bullet_arr.elementAt(i).Execute();
            }
        }
    }
    /*----------------------------------------------------------------------
    ...
    public static boolean isClearedAll(){
        for(int i=0;i<enemy_arr.size();i++){
            return false;
        }
        return true;
        ...
        switch (id) {
            case 1:
                if ((System.currentTimeMillis() - t) > 1000 - Wave * 10) {
                    Bullet_Multiplayer.Bullet_Reallo(x, y, -X / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo( x - 10, y, -X / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo(x + 10, y, -X / (float) D, -Y / (float) D, 1);
            ...
            case 3:
                if ((System.currentTimeMillis() - t) > 1000) {
                    Bullet_Multiplayer.Bullet_Reallo(x, y, -X / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo( x - 20, y, -(X - 20) / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo( x + 20, y, -(X + 20) / (float) D, -Y / (float) D, 1);
                    t=System.currentTimeMillis();
                }
                break;
