...
public class Enemy_Multiplayer extends GameObj {
    ...
    public static void render(Vector<Enemy_Multiplayer> enemy_arr, SpriteBatch batch) {
    ...
    public static void fire(Vector<Enemy_Multiplayer> enemy_arr, Vector<Bullet_Multiplayer> bullet_arr){
        for (int i=0; i<enemy_arr.size(); i++){
            if (enemy_arr.elementAt(i).State)
    ...
    public static void Enemy_Reallo(Vector<Enemy_Multiplayer> enemy_arr, float x_c, float y_c, int id, int Wave) {
        int EnemyGen = 0;
    ...
    public static void checkCollision(Vector<Enemy_Multiplayer> enemy_arr, Vector<Bullet_Multiplayer> bullet_arr){
        for (int i=0;i<enemy_arr.size();i++) {
            if (enemy_arr.elementAt(i).State) {
    ...
    public void checkCollision(Vector<Bullet_Multiplayer> bullet_arr){
        ...
        switch (id) {
            case 1:
                if ((System.currentTimeMillis() - t) > 1000 - Wave * 10) {
                    Bullet_Multiplayer.Bullet_Reallo(bullet_arr, x, y, -X / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo(bullet_arr, x - 10, y, -X / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo(bullet_arr, x + 10, y, -X / (float) D, -Y / (float) D, 1);
            ...
            case 3:
                if ((System.currentTimeMillis() - t) > 1000) {
                    Bullet_Multiplayer.Bullet_Reallo(bullet_arr, x, y, -X / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo(bullet_arr, x - 20, y, -(X - 20) / (float) D, -Y / (float) D, 1);
                    Bullet_Multiplayer.Bullet_Reallo(bullet_arr, x + 20, y, -(X + 20) / (float) D, -Y / (float) D, 1);
                    t=System.currentTimeMillis();
                }
                break;
